...
void LCD_function_set(void){
  LCD_write((_BV(LCD_FUNCTION)) | (_BV(LCD_FUNCTION_8BIT)) | (_BV(LCD_FUNCTION_2LINES)), 0); // 8-bit; 2-line; 5x8 font

  _delay_us(37);
}

void LCD_display_control(void){
  LCD_write((_BV(LCD_ON)) | (_BV(LCD_ON_DISPLAY)) | (_BV(LCD_ON_CURSOR)), 0); // display on; static cursor
